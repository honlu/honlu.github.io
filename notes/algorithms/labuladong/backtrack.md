...
date: 2026-08-31
category: Algorithms
tags: [算法, 回溯, DFS, labuladong]
summary: 回溯 = DFS + 剪枝，全排列 / N 皇后等经典问题的解题模板。
---
> 回溯 = DFS + 剪枝，全排列 / N 皇后等经典问题的解题模板。
...
func backTrack(nums []int, track []int) {
	...
	for i := 0; i  PS：皇后可以攻击同一行、同一列、左上左下右上右下四个方向的任意单位。
...
func solveNQueens(n int) [][]string {
	...
	// 棋盘定义和初始化
	board := make([][]string, n)
	for i := 0; i = 0 && j = 0 && j >= 0; i, j = i-1, j-1 {
		if board[i][j] == "Q" {
			return false
		}
	}
