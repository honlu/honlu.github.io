---
title: Redis 面试题：数据结构、持久化、高可用、缓存问题
date: 2026-08-31
category: Database
tags: [Redis, 面试]
summary: Redis 面试题整理，覆盖五大基础类型、持久化、主从复制、哨兵、Cluster、缓存雪崩/穿透/击穿等。
---
> Redis 面试题整理，覆盖五大基础类型、持久化、主从复制、哨兵、Cluster、缓存雪崩/穿透/击穿等。

# 开篇碎碎念

整理自 *Go-Interview* 仓库的 Redis 模块，覆盖 Redis 五大数据结构、持久化、高可用、缓存问题等高频面试题。每道题都附上「原理 + 命令示例」。

> 适合人群：需要理解 Redis 底层模型、准备后端 / SRE 面试的同学。
# 概述
