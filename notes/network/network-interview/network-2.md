---
title: 计算机网络面试题（下）：应用层（HTTP/HTTPS/DNS）、网络安全
date: 2026-08-31
category: Network
tags: [计算机网络, 面试]
summary: 计算机网络面试题下半部分，覆盖应用层（HTTP/HTTPS/DNS）、网络安全、CDN 等。
---
> 计算机网络面试题下半部分，覆盖应用层（HTTP/HTTPS/DNS）、网络安全、CDN 等。

# 开篇碎碎念

整理自 *Go-Interview* 仓库的 计算机网络 模块，分上下两篇覆盖 OSI/TCP/IP 各层协议、HTTP/HTTPS、DNS、CDN 等高频考点，附带小林 coding 的总结。

> 适合人群：需要复习计算机网络协议栈、准备后端面试的同学。
##  1. HTTP常见的状态码有哪些？
